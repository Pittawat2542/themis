---
title: Reproduce and replay runs
diataxis: how-to
...
sequenceDiagram
    ...
    participant E as "Experiment.replay(from_stage=Stage.JUDGE)"
...
| Rerun workflow-backed metrics in place | Generation stays fixed and only judge outputs should change | Requires stored upstream artifacts and judge access | `Experiment.replay(from_stage=Stage.JUDGE)` |
| Rerun pure scoring from parsed outputs | Parsing is fixed and deterministic scoring should be recomputed | Only useful when upstream parsing is already good | `Experiment.replay(from_stage=Stage.SCORE)` |
...
- [Reproducibility and replay](../explanation/reproducibility-and-rejudge.md)
