...
class WinLossMatrix:
    ...
    def to_table(self) -> str:
        ...
        for i, run_id in enumerate(self.run_ids):
            row = f"{run_id:<20} | "
            row += " | ".join(
                f"{self.matrix[i][j]:<12}" for j in range(len(self.run_ids))
            )
            lines.append(row)
...
class ComparisonReport:
    ...
    def summary(self, include_details: bool = False) -> str:
        ...
        # Overall summary
        lines.append(
            f"Comparing {len(self.run_ids)} runs across {len(self.metrics)} metrics"
        )
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "win_loss_summary": {
                metric: {
                    "rankings": [
                        {"run_id": rid, "wins": w, "losses": losses, "ties": t}
                        for rid, w, losses, t in matrix.rank_runs()
                    ]
                }
                for metric, matrix in self.win_loss_matrices.items()
            },
