...
def _resolve_humaneval_definition(normalized: str) -> BenchmarkDefinition:
    family = normalized.split(":", 1)[0]
    return build_humaneval_definition(family, normalized)


def _resolve_hle_definition(normalized: str) -> BenchmarkDefinition:
    if normalized == "hle":
    ...
    raw_variant_ids = [part.strip() for part in normalized.split(":", 1)[1].split(",")]
...
def _resolve_mmmlu_definition(normalized: str) -> BenchmarkDefinition:
    config_name = normalized.split(":", 1)[1].strip()
    supported_configs = {name.lower(): name for name in supported_mmmlu_config_names()}
    if config_name not in supported_configs:
        raise ValueError(
            f"Built-in benchmark 'mmmlu' received unknown config name: {config_name}"
        )
    return build_mmmlu_definition(supported_configs[config_name])


def _resolve_procbench_definition(normalized: str) -> BenchmarkDefinition:
...
def _resolve_rolebench_definition(normalized: str) -> BenchmarkDefinition:
...
def _resolve_superchem_definition(normalized: str) -> BenchmarkDefinition:
...
_VARIANT_RESOLVERS = {
    "hle": _resolve_hle_definition,
    "humaneval": _resolve_humaneval_definition,
    "humaneval_plus": _resolve_humaneval_definition,
    "mmmlu": _resolve_mmmlu_definition,
    "procbench": _resolve_procbench_definition,
    "rolebench": _resolve_rolebench_definition,
    "superchem": _resolve_superchem_definition,
}
...
def get_catalog_benchmark(name: str) -> BenchmarkDefinition:
    ...
    family = normalized.split(":", 1)[0]
    resolver = _VARIANT_RESOLVERS.get(family)
    if family in {"humaneval", "humaneval_plus"} and resolver is not None:
        return resolver(normalized)
    if (
        family == "hle"
        and resolver is not None
        and (normalized == family or ":" in normalized)
    ):
        return resolver(normalized)
    if resolver is not None and ":" in normalized:
        return resolver(normalized)
