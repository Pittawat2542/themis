...
def evaluate(
    *,
    ...
    _raise_if_running_loop(
        "evaluate() cannot be called from a running event loop. Use await themis.evaluate_async()."
    )
...
async def evaluate_async(
    *,
    ...
    return await experiment.run_async(
        store=store,
        subscribers=subscribers,
        tracing_provider=tracing_provider,
    )
...
def _build_experiment(
    *,
    ...
    data: Dataset | Sequence[Dataset] | Sequence[Mapping[str, Any]],
    metric: object | Sequence[object],
    parser: object | Sequence[object] | None,
    judge: object | Sequence[object] | None,
    samples: int,
    reducer: object | None,
    storage: StorageConfig | None,
    runtime: RuntimeConfig | None,
    seeds: list[int] | None,
    workflow_overrides: dict[str, object] | None,
    judge_config: dict[str, object] | None,
    environment_metadata: dict[str, str] | None,
    themis_version: str | None,
    python_version: str,
    platform: str,
) -> Experiment:
    ...
    effective_reducer = reducer or "builtin/majority_vote"

    return Experiment(
        generation=GenerationConfig(
            generator=cast(GeneratorComponent, model),
            candidate_policy=candidate_policy,
            reducer=cast(ReducerComponent | None, effective_reducer),
        ),
