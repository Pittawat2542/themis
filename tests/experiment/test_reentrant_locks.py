...
def test_reentrant_lock_same_thread(tmp_path):
    ...
    with storage._acquire_lock("run-1"):
        ...
        with storage._acquire_lock("run-1"):
            # And once more
            with storage._acquire_lock("run-1"):
                _, count = storage._lock_manager._locks["run-1"]
                assert count == 3

    # Verify lock was fully released
    assert "run-1" not in storage._lock_manager._locks
...
def test_reentrant_lock_count_tracking(tmp_path):
    ...
    with storage._acquire_lock("run-1"):
        assert "run-1" in storage._lock_manager._locks
        fd1, count1 = storage._lock_manager._locks["run-1"]
        assert count1 == 1

        with storage._acquire_lock("run-1"):
            fd2, count2 = storage._lock_manager._locks["run-1"]
            assert fd2 == fd1  # Same file descriptor
            assert count2 == 2  # Count incremented

            with storage._acquire_lock("run-1"):
                fd3, count3 = storage._lock_manager._locks["run-1"]
                assert fd3 == fd1
                assert count3 == 3

            # After inner context exits
            fd4, count4 = storage._lock_manager._locks["run-1"]
            assert count4 == 2  # Count decremented

        # After middle context exits
        fd5, count5 = storage._lock_manager._locks["run-1"]
        assert count5 == 1
...
def test_multiple_different_runs_different_locks(tmp_path):
    ...
    with storage._acquire_lock("run-1"):
        ...
        with storage._acquire_lock("run-2"):
            assert "run-1" in storage._lock_manager._locks
            assert "run-2" in storage._lock_manager._locks

            fd2, count2 = storage._lock_manager._locks["run-2"]
...
def test_os_compatibility_no_fcntl(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("themis.storage.locking.FCNTL_AVAILABLE", False)
...
def test_reentrant_lock_stress_test(tmp_path):
    ...
    def recursive_lock(depth: int):
        ...
        with storage._acquire_lock("run-1"):
            if depth == max_depth:
                fd, count = storage._lock_manager._locks["run-1"]
                assert count == 1
...
def test_lock_timeout_actual_wait(tmp_path):
    ...
    pytest.skip(
        "Timeout test requires 30s wait - tested in test_lock_timeout_on_external_lock"
    )
