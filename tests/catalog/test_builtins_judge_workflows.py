...
def test_catalog_builtin_judge_metrics_build_expected_workflows() -> None:
    ...
    candidate = GenerationResult(candidate_id="case-1-reduced", final_output={"answer": "4"})
    pair_a = GenerationResult(candidate_id="case-1-candidate-0", final_output={"answer": "4"})
    pair_b = GenerationResult(candidate_id="case-1-candidate-1", final_output={"answer": "5"})
    ...
    llm_workflow = llm_rubric.build_workflow(CandidateSetSubject(candidates=[candidate]), ctx)
    panel_workflow = panel.build_workflow(CandidateSetSubject(candidates=[candidate]), ctx)
    majority_workflow = majority.build_workflow(CandidateSetSubject(candidates=[candidate]), ctx)
    pairwise_workflow = pairwise.build_workflow(CandidateSetSubject(candidates=[pair_a, pair_b]), ctx)

    assert len(llm_workflow.judge_calls()) == 2
    assert "grade factual accuracy" in llm_workflow.render_prompt(
...
def test_catalog_builtin_judge_metrics_run_end_to_end_through_experiment() -> None:
    experiment = Experiment(
        ...
        evaluation=EvaluationConfig(
            metrics=["builtin/llm_rubric", "builtin/pairwise_judge", "builtin/panel_of_judges", "builtin/majority_vote_judge"],
