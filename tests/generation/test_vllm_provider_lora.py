...
def fake_vllm_module_with_lora(monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "vllm.lora", types.SimpleNamespace(request=lora_request_module))
...
def test_vllm_provider_lora_request(monkeypatch):
    ...
    engine = provider._engines[0]  # Access the engine (assuming single engine or RR index 0 was used)
