...
from .config import OpenAIExampleExperimentConfig, ModelConfig


def create_project_experiment(
    config: OpenAIExampleExperimentConfig,
) -> ProjectExperiment:
    """Create a project experiment from the OpenAI example configuration."""
    ...
    project_experiment = ProjectExperiment(
        name="openai-example-math-experiment",
        description="OpenAI example math experiment using JSON extraction",
...
def run_experiment(
    ...
    if not dataset_rows:
        raise ValueError("OpenAI example experiment requires at least one dataset row")

    # Apply n_records limit if specified
    if config.n_records is not None:
        dataset_rows = dataset_rows[: config.n_records]
    ...
    # Use progress reporter for generation and evaluation
