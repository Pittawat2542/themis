...
def analyze_prompt_effectiveness(csv_path: Path) -> Dict[str, float]:
    ...
    strategy_col = 'template_strategy' if 'template_strategy' in df.columns else 'prompt_strategy'
    
    # Look for accuracy column - based on the CSV it's 'metric:ExactMatch'
    accuracy_col = 'metric:ExactMatch' if 'metric:ExactMatch' in df.columns else 'exact_match'
...
def compare_models_by_prompt(csv_path: Path) -> pd.DataFrame:
    ...
    if model_col in df.columns and strategy_col in df.columns and accuracy_col in df.columns:
        comparison = df.groupby([model_col, strategy_col])[accuracy_col].mean().unstack()
        return comparison
    else:
        print("Required columns not found in CSV")
        print("Available columns:", df.columns.tolist())
...
def generate_comparison_report(csv_path: Path, output_path: Path) -> None:
    ...
    with open(output_path, 'w') as f:
        ...
        if model_col in df.columns and strategy_col in df.columns and accuracy_col in df.columns:
            cross_analysis = df.groupby([model_col, strategy_col])[accuracy_col].mean().unstack()
            try:
